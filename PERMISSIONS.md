...
| Account | `upd` | Optional. Applies the product's inode, open-file and process limits when a package changes. Without it the package still moves; only those three limits are skipped. Some servers refuse this call even with the grid switched on — CWP checks it as `accout_upd` from code that cannot be inspected. If yours does, set `apply_resource_limits => false` in `config.php` so the module stops attempting it. |
